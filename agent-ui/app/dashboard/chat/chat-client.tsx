...
export default function ChatClient({
    ...
    const { messages, isStreaming, activeAgent, activeAgentUrl } = useSelector(
    ...
    useEffect(() => {
        ...
        if (initialSession) {
            // Hydrate session
            // We manually dispatch actions to populate the store instead of calling loadSession async thunk again
            // to avoid double fetching, or we can use the thunk if we didn't pass full session data.
            // But since we have the data, let's just set it.
            // However, chat-slice doesn't expose a simple "setFullState" action easily without extra reducers.
            // Easiest way with existing slice:
            dispatch(setSessionId(initialSession.id));
            dispatch(setActiveAgent({ url: initialSession.agentUrl, agent: initialAgent || null }));

            // We need to populate messages. The slice has `addMessage`.
            // We should probably clear existing messages first if any.
            dispatch(resetChat());
            // Re-set session ID and Agent which resetChat cleared
            dispatch(setSessionId(initialSession.id));
            dispatch(setActiveAgent({ url: initialSession.agentUrl, agent: initialAgent || null }));

            // Populate messages
            initialSession.messages.forEach(msg => {
                dispatch(addMessage(msg));
            });

        } else if (agentUrl && initialAgent) {
            // New Chat
            dispatch(resetChat());
            dispatch(setActiveAgent({ url: agentUrl, agent: initialAgent }));
        }
    }, [initialSession, initialAgent, agentUrl, sessionId, dispatch]);
    ...
    const handleSubmit = (e?: FormEvent) => {
        e?.preventDefault();
        if (!inputText.trim() || (!activeAgentUrl && !sessionId) || isStreaming)
            return;
    ...
    return (
        <div className="flex flex-1 flex-col h-full overflow-hidden relative">
                        ...
                        <div className="flex flex-col items-center justify-center text-center mt-20 px-4">
                            ...
                            <p className="text-muted-foreground max-w-md mx-auto">
                                {activeAgent
                                    ...
                                    "Start a conversation to see what this agent can do."
                                    : "Choose an agent from the sidebar to start a new chat."}
                            </p>
            ...
            {/* Floating Input Area */}
            <div className="absolute bottom-0 left-0 right-0 p-4 bg-linear-to-t from-background via-background to-transparent pt-10">
                <div className="max-w-4xl mx-auto">
                    <form onSubmit={handleSubmit} className="relative">
                        <InputGroup className="shadow-lg border-primary/20 bg-card rounded-xl overflow-hidden ring-offset-background focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 transition-all">
                            <InputGroupTextarea
                                placeholder={
                                    activeAgent ? "Ask anything..." : "Select an agent first..."
                                }
                                value={inputText}
                                onChange={(e) => setInputText(e.target.value)}
                                onKeyDown={handleKeyDown}
                                disabled={!activeAgentUrl || isStreaming}
                                className="min-h-15 max-h-50 border-0 focus-visible:ring-0 resize-none py-4"
                            />
                            <InputGroupAddon align="block-end" className="pr-2 pb-2">
                                <InputGroupText className="text-xs text-muted-foreground mr-2 self-end pb-2">
