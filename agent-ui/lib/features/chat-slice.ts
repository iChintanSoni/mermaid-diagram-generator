...
import { AgentCard, MessageSendParams, Message, TaskStatusUpdateEvent, TaskArtifactUpdateEvent } from "@a2a-js/sdk";
import { ClientFactory } from "@a2a-js/sdk/client";
import { ChatSession } from "@/lib/db";
import { RootState } from "@/lib/store";

// Define union type for events we want to store
export type StreamEvent = Message | TaskStatusUpdateEvent | TaskArtifactUpdateEvent;
...
>("chat/sendMessageStream", async ({ text, agentUrl }, { dispatch, getState, rejectWithValue }) => {
    try {
        ...
        const userMsg: ChatMessage = { id: userMsgId, role: "user", content: text, events: [] };
        dispatch(addMessage(userMsg));

        // 2. Add Assistant Placeholder (empty initially)
        const assistantMsgId = crypto.randomUUID();
        const assistantMsg: ChatMessage = { id: assistantMsgId, role: "assistant", content: "", events: [] };
        ...
        for await (const chunk of client.sendMessageStream(params)) {
            ...
            // store every event
            dispatch(addEventToMessage({ messageId: assistantMsgId, event: chunk as StreamEvent }));
            ...
            if (event.kind === "message") {
                ...
                if (messageInChunk.parts) {
                    ...
                    dispatch(updateMessageContent({ id: assistantMsgId, content: fullContent }));
                }
            }
        }
...
export const chatSlice = createSlice({
    ...
    initialState,
    reducers: {
        setActiveAgent: (state, action: PayloadAction<{ url: string; agent: AgentCard | null }>) => {
        ...
        addEventToMessage: (state, action: PayloadAction<{ messageId: string; event: StreamEvent }>) => {
...
export const { setActiveAgent, setSessionId, addMessage, updateMessageContent, addEventToMessage, setStreaming, resetChat } = chatSlice.actions;
